...
description: 封闭式整改验收 skill。用于一次开放式对抗评审完成并回补后，核验原采纳项与授权裁决是否被完整、无误、无越界地落实。只调用当前运行环境一个全新上下文的只读检查者，再由主线程逐条裁决；禁止发现新设计问题、禁止重开已决策、禁止再次运行多模型对抗评审。触发：评审整改验收、评审回补检查、关闭对抗评审、确认整改没有改变设计语义。
---
...
| 环节 | 目的 | 模型 |
|---|---|---|
| 开放式对抗评审 | 找未知漏洞、暴露不同视角 | 触发时由用户从本机可用 Claude Code、Cursor Agent、Codex 模型中选择至少一个；主线程直接裁决 |
...
使用当前运行环境的一名全新上下文检查者；若当前环境没有隔离上下文能力，可从对抗评审中已选且仍可用的桥接调用**一个**只读模型。模型与桥接记录实际值；不重新开放模型竞赛或设计评审。调用方式遵循 `adversarial-review` 的只读边界。
...
- 驳回必须留下“检查者意见 / 不成立原因 / 依据 / 是否改变业务结果”，不得凭直觉否决。
...
| CR | 关联 AR/DEC | 检查者意见 | 主线程裁决 | 依据 | 修正 |
|---|---|---|---|---|---|
