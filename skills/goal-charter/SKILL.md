...
**启动服务不是准入闸**：章程阶段同时清算已经可预见的用户停机，产出 `_shared/T{n}-goal启动服务单.md`。它只保存当前环境、会话、凭据位置和外部平台状态，不能签发 PASS/FAIL、冻结命令或削弱 M0 自愈；真正约束 goal 的仍只有上游授权与本章程边界。
...
> 只有 AI 无法操作的真实设备、原生客户端对象或原生平台动作才可成为用户里程碑；AI 可控浏览器/开发者工具、DOM/data 断言、截图与 VLM 判读仍属自动化，不得转嫁给用户。
> 用户里程碑只能来自 §4.1 分类后的 `CANDIDATE_DEPENDENT`；已经完成的 `PRE_GOAL` 不得继续出现在本表。

## 3. 权限边界
### 3.1 允许（无需请示，做了记日志）
1. 继承上游授权（不得扩大）：目标环境 `{}` / 精确动作 `{}` / 目标对象 `{}` / 费用上限 `{含重试储备}` / 副作用范围 `{}` / 授权锚点 `{DEC-x或总控批准记录}`
2. goal 可在授权范围内创建或修复：产品代码、测试脚本、runner、夹具、数据建立/清理/恢复工具、环境适配器、部署脚本与证据采集器；每次修复登记影响面并重跑受影响验证
3. 测试资产修改的硬不变量：真实断言集合不得弱于冻结规格，不得用 skip、放宽阈值或把产品现状反写测试来求绿
### 3.2 禁止（硬边界，违反即失败）
1. …（**按可达性写，不按动作枚举**，见 §5）

## 4. 停机条件（白名单，仅此三条需要用户裁决）
...
**停机后的处置（默认热修续跑，回炉是例外）**：停机报告必须随附一份**修改方案**——①决策项（同族冲突一次收齐，互斥选项 + 影响，沿用冲突批处理合并决策包格式）②上游修改清单（要动的轻量设计条目 / 正式 L1–L7 小节 / L7 用例，逐项改前→改后语义）③失效证据清单（按影响面圈定重跑范围，未受影响证据不作废）④恢复点（从哪个断点、哪个切片继续）⑤档位建议（热修为默认；建议回炉必须论证热修为何不可靠——地基级设计错误、影响面无法圈定）。**用户批准一次 = 一揽子授权**：方案内全部上游修改、五项冻结闸门重跑、受影响证据重跑、断点续跑同一 goal，中间不再确认。热修复用有界重新冻结的同一套机器，语义变更以本次批准为裁决来源；完成后更新断点与章程引用的哈希基线（登记动作，不重写章程），用**同一条 /goal 条件**重新启动续跑。

## 5. 回炉出口（例外档，用户点头才走）
回炉不再是规格错误的默认出口——默认走 §4「修改方案 → 一次批准 → 热修续跑」。仅当修改方案论证热修不可靠（地基级设计错误、影响面无法圈定）且用户确认时：带证据终止本 goal → 回退上游子任务重新设计 → 修用例 → 重写章程 → 重跑。
判据：…（本次哪些属规格层，见 §6）

## 6. 已知豁免（不算失败；每条三段：豁免项 / 原因 / **验收时你将看到什么现象**）
1. …（"现象"用人话写给验收者，例：「第三方支付回调永远不来，N 分钟后一律判超时」——不写现象 = 用户在终点才发现"全绿"和"能用"不是一回事）
...
| `PRE_GOAL` | 必须用户参与，但不依赖候选代码或部署结果 | 章程阶段提前完成；用户明确选择延期时才可转成用户里程碑 |
| `CANDIDATE_DEPENDENT` | 只有候选部署或运行后才能真实完成 | 留在 goal，并把可合并事项收成最少用户窗口 |
| `LONG_LATENCY_EXTERNAL` | 审批、开户或外部协调周期可能跨天 | 上游独立任务完成，不埋入 goal |

伴生服务单至少记录：唯一环境与授权锚点、访问入口和登录/权限状态、凭据是否存在及**受控位置**（禁止明文）、外部平台快照、每项分类与依赖、已完成/明确延期事实、回滚入口、`planned_user_windows` 与不可合并原因。默认目标为 0；确实依赖候选时目标为 1；超过 1 只要求解释和合并复核，**不形成机械闸门**。

服务单是可刷新现场基线，不是执行契约。会话过期、路径变化、命令失效或工具缺失仍由 M0 在授权内修复；只有缺权、找不到凭据位置或真实物理边界才按章程出口处理。章程拍板前尚未完成的 `PRE_GOAL` 应当当场清算；若用户明确选择延期，记录原话/日期/影响后转入对应用户里程碑，不得默认延期。
...
| **规格错了 / 洞会改变业务结果** | 需要用户裁决的语义修改 | 停机出 §4 **修改方案**，一次批准后热修续跑（默认）；方案论证热修不可靠且用户确认 → §5 回炉 |
...
> 停机 ≠ 回炉：停机后默认按 §4 修改方案热修续跑（批准即续，不再确认）；回炉是**本次 goal 作废，退回上游子任务重来**——例外档，用户点头才走。
...
- **部署动作读写部署账本**（目标环境有账本机制时，位置与格式见项目部署 skill）：**用户触发 goal 即视为对目标环境的当次使用授权**——账本显示被其它任务/分支占用一律视为过时登记信息，**不停机**、按授权继续部署；部署后必登记一行（时间/分支/commit/任务/操作者），**手动或热修部署同规登记**。登记是唯一的审计义务；环境使用规范化是独立改进项，落地前以本条为准。
...
- 判失败：目标环境存在部署账本机制，而部署类里程碑未包含「部署后登记账本」步骤；或把「账本显示被占用」写成停机理由（触发 goal 即当次环境授权，占用信息一律视为过时）。
- 判失败：停机条件超出三条白名单；或把凭据定位、实现取舍、重复失败、"文档冲突""脚本小错""记录笔误"写成用户裁决理由。
...
- 判失败：AI 可操作事项被降级为用户里程碑；候选无关的 `PRE_GOAL` 未完成且用户未明确选择延期便进入 goal；可合并的候选依赖事项被拆成多个用户窗口而无客观理由。
...
| 风险触发或用户点选的章程开放式红队 | `adversarial-review`（章程口径卡） |
...
命中章程 §4 白名单三条 / 触发 §5 回炉 / 到达用户里程碑 {M-x} / 已跑满 {N} 轮并交付 EXECUTION_BLOCKED。
```
...
§5 回炉 / 到达用户里程碑 {M-x} / 已跑满 {N} 轮并交付 EXECUTION_BLOCKED。
```
...
```text
写章程 → （风险触发或用户点选时：红队一次 → 封闭验收 PASS）→ 用户拍板
