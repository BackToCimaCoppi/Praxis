...
**项目绑定**：编写章程前按 `task-control-doc/references/标准研发流程.md` §6.1 解析当前项目唯一完整的项目研发流程适配器。此时只读能力、权威来源和不支持项，不申请环境或租约。执行段启动后才读取实际占用。适配器不能替代业务、环境或发布授权。

**目标环境必须由具权角色选择**：继承本任务已有的环境选择原话和授权锚点；尚未选择时列出项目实际支持的环境、能力差异与推荐，请环境所有者或发布授权人选定后再定稿。章程作者不得默认沙盒、测试或生产，也不得用整份章程获批倒推环境选择。

## 1. 铁律（写章程的人最容易违反的三条）

1. **章程无权铸造规则**。规则只来自用户级文档（项目 `AGENTS.md` / skill）。章程里写的"硬门""死亡线""必须审批"**一律只是执行建议，不具停机权**。你在写一份执行契约，不是在立法。
...
3. goal 可在授权范围内创建或修复：产品代码、测试脚本、runner、夹具、数据建立/清理/恢复工具、环境适配器、部署脚本与证据采集器；每次修复登记影响面并重跑受影响验证；新建或实质修改取证工具按 `test-execution-router §3.6` 在 Goal 内独立核对
...
**停机后的处置（默认热修续跑，回炉是例外）**：停机报告必须随附一份**修改方案**——①决策项（同族冲突一次收齐，互斥选项 + 影响，沿用冲突批处理合并决策包格式）②上游修改清单（要动的轻量设计条目 / 正式 L1–L7 小节 / L7 用例，逐项改前→改后语义）③失效证据清单（按影响面圈定重跑范围，未受影响证据不作废）④恢复点（从哪个断点、哪个切片继续）⑤档位建议（热修为默认；建议回炉必须论证热修为何不可靠——地基级设计错误、影响面无法圈定）。**对应业务决策负责人批准一次 = 方案范围内的一揽子授权**：方案内全部上游修改、五项冻结闸门重跑、受影响证据重跑、断点续跑同一 goal，中间不再确认。若项目要求多角色会签或职责分离，则按项目治理策略完成，不得由任务发起人替代。热修复用有界重新冻结的同一套机器，语义变更以本次批准为裁决来源；完成后更新断点与章程引用的哈希基线（登记动作，不重写章程），按同一章程和当前运行时启动方式续跑。
...
章程作者在生成自主执行启动指令前，把每个密钥、登录、第三方后台、真实设备和外部平台事项分到唯一类别：
...
候选验收不能只核“命令成功、哈希闭合、账本写 PASS”。对高风险或新改取证工具，必须从原始报告定位到具体观测，将 observed 与冻结 expected 或判定器实际比较；证明失败事实可被拒绝。普通 AC 可直接使用项目原生断言和报告，避免为包装而造第二套事实。执行细则与正反例核对见 `test-execution-router` §3.5—3.6。
...
每个任务都一样，所以它属于本 skill；章程实例不要抄一遍。批量收集、集中修复、完整核验及效率与质量分离统一按 `test-execution-router §3.2—3.4`，原断点/飞行日志记录累计修复—复验次数。
...
- **goal 内禁止开放式对抗评审**。常规自检使用当前工具与机器检查器；新建或实质改变取证工具时按 `test-execution-router §3.6` 做一次全新上下文、只读、有界的独立核对，不重开业务设计。
...
- **动作授权与实际占用分别核验**：章程只继承有权限角色对环境、动作、对象、费用和副作用的明确授权；执行段通过项目适配器绑定的生命周期入口取得实际占用。授权不等于租约，历史部署账本也不能替代当前占用核验。一次部署、回滚或测试结束不代替整个执行段的收尾与释放。
...
- ✅ 章程已枚举的独立施工、验证切片，以及原授权内本轮收齐失败后按根因枚举的修复包，至少两个且有可用槽位时按 `test-execution-router §3.3` 并行；真实依赖、共享写入或资源冲突才排队
- ❌ **禁二级派生**（子执行体不得再派）
...
**交接类**（详见 §13.6）
- 判失败：启动物不是一条指向章程的自主执行指令（做成了提示词+条件两块）、复制章程正文、②段缺举证义务、④段漏出口或把停机写成"等待某人继续执行"、在章程获批前生成，或回填后总控脚本抓不到。
...
> **轻量档例外（plan-goal）**：`plan-goal` 轻量任务的计划内嵌「执行章程节」，不是本 skill 的章程实例——是否对计划跑开放式红队由用户决定，AI 不主动触发；但 §13 的运行时适配与出口写法对轻量档同样生效。
...
章程由业务决策负责人或项目指定角色批准后，生成**一条指向章程与工作包的启动指令**。具体启动工具由项目补丁在运行时指定：可以是当前 Agent 的 goal 能力、会话续跑机制或项目自带的执行器；不能把某一产品的命令、参数上限、自动续轮、审批模式假定为所有运行时共有。

### 13.1 启动前核实

1. 查询当前运行时是否支持目标跟踪、自动续轮、断点恢复、停止、工具审批与预算限制；把实际能力和缺项写进章程或工作包。
2. 没有自动续轮时，仍按章程逐里程碑执行；上下文中断后从磁盘重读章程和断点，不假称工具会自行续跑。
3. 若评估器只能读会话内容，终态的原始命令结果和产物位置必须在会话中可见；若能读取文件，也须保留可审计原件。不得只口称通过。
4. 将停机白名单、规格过时、人工里程碑、限轮或预算耗尽映射成**当前执行器真正会停止**的出口；无法实现时不用无人值守模式。

### 13.2 启动指令四段

~~~text
执行 {工作包路径}。首次启动、恢复、新会话或上下文压缩后，先从磁盘重读 {章程路径}、工作包必读项、断点与执行日志；章程是唯一执行契约，按授权的有界切片并行推进。
完成条件：{章程 §1 的每条可判定终态 + 原始证据位置/实际输出}；候选 commit/tree、工作树状态与文档/测试结果可核；只报 CANDIDATE_READY。
约束：{章程 §3.2 最关键的 3–6 条，不复制全文}。
出口：{章程 §4 停机白名单、§5 回炉、人工里程碑、轮次/预算上限；任一命中立即停止并报告状态}。
~~~

启动指令只放下游工作包的「会话启动提示词」段，标题后紧跟代码块，供总控脚本提取。不得另造一份相互漂移的执行契约。启动指令由终版章程生成；若风险评审或项目治理要求先评审和封闭验收，须 PASS 并获授权后再生成。

### 13.3 失败条件

- 启动工具实际不能停止却宣称停机出口有效，或无法验证自动续轮却宣称无人值守。
- 完成条件没有原始举证义务，只有主观“已通过”。
- 章程未获批准就启动，或把章程正文复制成第二套真值。
- 有界并行授权被遗漏，或越过项目角色、环境租约和发布授权。
- 回填后总控脚本无法提取启动段，或改动了下游工作包其他内容。
