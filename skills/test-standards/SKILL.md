...
- 手工验证必须有物理边界证明、runbook、客观观测点、回传材料和计划内用户里程碑。
- 跳过任一测试类型时，必须说明本次改动为什么不命中，而不是写“暂不需要”。
- 每次测试矩阵对下列八面**恰好各有一行**；不命中写可核 N/A 理由，禁止省略行。
...
| 小程序视觉验收 | 页面布局、示例内容、图表、canvas、视觉规格变化 | 自动产出截图并由 VLM/视觉模型判读规格差异；不能只证明截图存在 |
| 手工验证 | AI 无法操作的真实设备、原生客户端对象、支付/OAuth/原生授权 | 仅作物理边界例外；物理边界证明 + runbook + 观测点 + 回传材料 + 用户里程碑缺一不可 |
...
- `VERIFIED` 只绑定套件成功、聚合哈希或换名后的通用观测，不能还原该 AC 的实际业务结果。
- 金标准只有正向返回值，没有不变量终态与负向副作用的原生观测事件。
