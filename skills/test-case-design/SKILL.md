...
| `execution_ref` | 目标测试文件/用例锚点；T4/T5 可填预定锚点，goal 的 M0 负责实现为可运行落点，后续可按冻结语义修复 |
| `manual_reason` | 仅手工用例填写：AI 无法操作的真实设备/原生对象/原生授权等物理边界证明；“视觉判断”“GUI”“交互式工具”不成立 |
| `manual_runbook` | 仅手工用例填写：用户步骤、客观观测点、回传材料与计划内用户里程碑 |
...
- UI 路径是否覆盖关键状态反馈。
- 视觉变化是否有判读点。
- 手工用例是否只用于 AI 无法替代的物理边界，且物理证明、runbook、回传材料与用户里程碑齐全；AI 可控 GUI/VLM 不得标手工。
