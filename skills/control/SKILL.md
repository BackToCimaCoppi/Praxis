...
9. **停机白名单封闭，只有三条**：①两份**都已冻结**的真值真矛盾且改变业务结果 ②命中项目死亡线清单的**业务规则**要变 ③不可逆 / 外发动作。**其余一律先修后报**——笔误、脚本 bug、过期描述、证据充分的简单缺陷，自己修 + 留痕 + 继续。**打断用户的成本远高于一次可回滚的自愈**。冲突批处理与 `EXECUTION_BLOCKED` 细则以总控规范 §4.5 为真值；goal 执行态以章程 §4 白名单为准（章程已预授权的动作不触发第 ③ 条）
