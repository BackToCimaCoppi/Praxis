...
7. **过程资产边界**：执行 Tn 时产生的中间文档（设计稿、清单、批判稿等），只能写到 `_shared/` 且文件名带 `T{n}-` 前缀（任务级共享资产无前缀）；**不可**用其他子任务的 `T{m}-` 前缀（m≠n），**不可**平铺到任务根目录，**不可**新建 `_T{n}/` 目录。详见总控规范 §1.1.1。
8. **规则铸造权不在工作包手上**：只有项目级指令文档与 skill 能铸造规则。工作包 / 评审报告 / goal 章程里的「硬门」「死亡线」「必须审批」**一律是执行建议，不具停机权**；**死亡线是封闭清单，任何人无权新增**——包括你（总控规范 §4.4）
9. **停机白名单封闭，只有三条**：①两份**都已冻结**的真值真矛盾且改变业务结果 ②命中项目死亡线清单的**业务规则**要变 ③不可逆 / 外发动作。**其余一律先修后报**——笔误、脚本 bug、过期描述、证据充分的简单缺陷，在预授权边界内修复 + 留痕 + 继续。减少临时协调不覆盖项目声明的授权角色、环境占用、职责分离或强制检查点。冲突批处理与 `EXECUTION_BLOCKED` 细则以总控规范 §4.5 为真值；goal 执行态以章程 §4 白名单为准（章程已由对应权限角色预授权的动作不触发第 ③ 条）
10. **获批总控可包含本地提交授权**：只有经任务治理负责人批准的总控，才预授权在其范围内 `git commit`；项目补丁可要求额外检查。**推送远端仍需发布授权人显式授权**（总控规范 §4.7）
...
4. **允许**：表格之后追加**最多一段**简短"下一步建议"，格式必须从下列固定模板选一条：
   ...
   - 「存在阻塞：`Tn`，请对应授权角色裁决」—— 派生状态 = 阻塞
   - 「未启动，请任务调度负责人决定从哪个子任务开始」—— 派生状态 = 未启动
