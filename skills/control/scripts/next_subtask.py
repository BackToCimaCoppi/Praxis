...
def render(task_dir: Path, doc: Path, picked: Subtask | None, reason: str) -> str:
    ...
    out.append("执行前重读子任务详情中的「强制阅读」清单，并主动补读背景（背景导航/其他子任务/相关文档；亲自读，不派子 agent）。goal 首次启动、--resume、新会话或自动上下文压缩后，第一次写入前还须从磁盘重读章程、goal 断点与飞行日志尾部。")
...
def render_session_prompt(task_dir: Path, doc: Path, picked: Subtask) -> str:
    ...
    if split_target:
        ...
        body = (
            ...
            f"2. 读取该文件「强制阅读」列出的文件；若为 goal 首次启动、--resume、新会话或自动上下文压缩后的续跑，第一次写入前同时从磁盘重读章程、goal 断点与飞行日志尾部；再主动补读「背景导航」和你自己判断需要的背景（父级 README、其他子任务、相关正式文档、代码现状），把背景挖够再动手\n"
        ...
        body = (
            ...
            f"3. 读取该子任务「强制阅读」列出的文件；若为 goal 首次启动、--resume、新会话或自动上下文压缩后的续跑，第一次写入前同时从磁盘重读章程、goal 断点与飞行日志尾部；再主动补读「背景导航」和你自己判断需要的背景（其他子任务详情、相关正式文档、代码现状），把背景挖够再动手\n"
