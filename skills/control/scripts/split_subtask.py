...
def child_file_skeleton(parent_seq: str, parent_name: str, child: ChildSpec, task_dir_name: str) -> str:
    ...
    return "\n".join([
        ...
        "",
        "【主体任务】（补全：一句话说明本子任务做什么、产出什么）",
        "【目标终态】（补全：完成判定的可度量压缩提要）",
        "【边界提要】（补全：「不做什么」关键禁令压缩；详细以本工作包为准）",
        "",
        "请按以下步骤：",
        "1. 读取本文件（含强制阅读、输入、要做的事情、不做什么、输出物、完成判定）",
        "2. 读取「强制阅读」列出的文件；再主动补读「背景导航」和你自己判断需要的背景（父级说明、其他子任务、相关文档、代码现状），把背景挖够再动手",
        "3. 自主补读必须亲自读（直接读文件/检索），不要为补背景派子 agent 或起深度调查",
        "4. 严格在本子任务范围内执行——读什么放开，做什么、写什么仍只限本子任务",
        "5. 完成后回填本文件的「当前状态」为已完成、「输出物」为实际产出文件",
        f"6. 同步更新 docs/00-任务总控/{task_dir_name}/README.md 子任务总表对应行 + 进展记录",
