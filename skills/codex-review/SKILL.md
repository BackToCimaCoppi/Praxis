...
| 触及上游已拍板决策 / 承重契约面 / 冻结规格本身错了 | **返闸上游**：默认热修——附修改方案经用户一次批准后修订设计/用例并回原执行续跑；热修不可靠且用户确认才回炉（退回设计步重来） |
...
- **下游 = 修完收口**：评审收敛、代码定稿。**没有"回补正式真值层"这一步**——规格已在施工前冻结；若核验证明规格本身错了，走 §6 返闸上游（默认热修，经用户批准修订规格；不可靠才回炉），不是私自反写规格。
