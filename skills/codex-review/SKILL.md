---
name: codex-review
description: 代码评审 skill（用户级通用）。施工完成后，由 codex 单模型对照已冻结的正式 L1-L7、规格物化覆盖报告、goal 章程与工程红线做低成本符合性核验；轻量设计只作 SD/DP 追溯证据，不与正式规格并列。仅在用户手动触发时运行，不挂门禁，也不替代高风险代码的多视角审查。
---
