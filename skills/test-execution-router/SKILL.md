...
涉及真实动作时必须有上游用户授权锚点，逐项锁定目标环境、精确动作、目标对象、费用上限（含重试储备）与副作用范围。没有独立 readiness 门票；`bootstrap` 的失败先在 goal 内自愈。只有两份冻结真值矛盾、死亡线业务规则要变、未预授权不可逆/外发动作或计划内真实人工里程碑才交给用户；安全路径穷尽或达到限轮只交付 `EXECUTION_BLOCKED`，不前拉上游。
...
| `SPEC_STALE` | 冻结用例或上游正式真值的语义确有缺陷 | 用例缺陷回测试用例设计；正式规格/业务语义缺陷回设计+规格冻结；按语义影响面作废下游 |
