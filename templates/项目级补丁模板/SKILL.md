...
description: 【模板·复制后改名】项目级补丁 skill 模板。Praxis 里的用户级 skill（doc-layer-system / lightweight-design / docs-from-code 等）只含通用引擎与"留白挂载点"，不硬编码任何项目特定值。本模板把所有挂载点列成填空区——复制到你项目的 .agents/skills/ 下、改名为 <你的项目>-patch、逐项填上你项目的真实值，引擎即可挂载运行。触发词：由你按需声明。
---
...
1. 把本目录复制到你项目仓库的 `.agents/skills/<你的项目>-patch/`
...
## 14. 项目研发流程适配器（总控 / goal-charter / plan-goal）

若本补丁负责研发流程绑定，在 description 或开头明确声明“项目研发流程适配器”。下列八槽均须填实际来源，或写“明确不支持 + 原因 + 受影响义务”；不能留空或用无解释 N/A。三个通用入口发现时只能选唯一完整适配器，不拼接多个候选。适配器只声明能力，不取得环境租约或动作授权。
...
- [ ] 协作 skill 名与你项目 `.agents/skills/` 下的真实目录名一致
